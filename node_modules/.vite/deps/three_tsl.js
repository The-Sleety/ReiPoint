...
var TextureNode = class extends UniformNode {
	...
	*/
	generateSnippet(builder, textureProperty, uvSnippet, levelSnippet, biasSnippet, depthSnippet, compareSnippet, gradSnippet, gatherSnippet, offsetSnippet, flipYSnippet) {
		...
		else if (gatherSnippet) if (compareSnippet) snippet = builder.generateTextureGatherCompare(texture, textureProperty, uvSnippet, compareSnippet, depthSnippet, offsetSnippet, flipYSnippet);
...
var MaterialNode = class MaterialNode extends Node {
	...
	*/
	setup(builder) {
		...
		else if (scope === MaterialNode.LIGHT_MAP) if (material.lightMap) node = this.getTexture(scope).rgb.mul(this.getFloat("lightMapIntensity"));
		else node = vec3$1(0);
		else if (scope === MaterialNode.AO) if (material.aoMap) node = this.getTexture(scope).r.sub(1).mul(this.getFloat("aoMapIntensity")).add(1);
		else node = float$1(1);
...
var instance$1 = /*@__PURE__*/ Fn$1(([matrices, colors = null], builder) => {
	...
	if (colors) if (isStorageColor) instanceColorNode = storage$1(colors, "vec3", Math.max(colors.count, 1)).element(instanceIndex$1);
	else {
...
Fn$1(({ material, renderer }) => {
	...
	if (useWorldUnits) {
		...
		if (!useDash) if (useAlphaToCoverage && renderer.currentSamples > 0) {
...
var StackNode = class extends Node {
	...
	*/
	addToStack(node, index = -1) {
		...
		if (index === -1) if (this._currentNode) {
