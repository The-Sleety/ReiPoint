import { Bs as Vector3, Nt as Curve } from "./three.module-DpBjf9BN.js";
//#region node_modules/three/examples/jsm/curves/CurveExtras.js
/**
...
var GrannyKnot = class extends Curve {
	...
	*/
	getPoint(t, optionalTarget = new Vector3()) {
		...
		t = 2 * Math.PI * t;
		const x = -.22 * Math.cos(t) - 1.28 * Math.sin(t) - .44 * Math.cos(3 * t) - .78 * Math.sin(3 * t);
		const y = -.1 * Math.cos(2 * t) - .27 * Math.sin(2 * t) + .38 * Math.cos(4 * t) + .46 * Math.sin(4 * t);
		const z = .7 * Math.cos(3 * t) - .4 * Math.sin(3 * t);
...
var HeartCurve = class extends Curve {
	...
	*/
	getPoint(t, optionalTarget = new Vector3()) {
		...
		return point.set(x, y, 0).multiplyScalar(this.scale);
	}
};
/**
* A Viviani curve.
...
var DecoratedTorusKnot4a = class extends Curve {
	...
	*/
	getPoint(t, optionalTarget = new Vector3()) {
		const point = optionalTarget;
		t *= Math.PI * 2;
		const x = Math.cos(2 * t) * (1 + .6 * (Math.cos(5 * t) + .75 * Math.cos(10 * t)));
		const y = Math.sin(2 * t) * (1 + .6 * (Math.cos(5 * t) + .75 * Math.cos(10 * t)));
		const z = .35 * Math.sin(5 * t);
		return point.set(x, y, z).multiplyScalar(this.scale);
	}
};
/**
...
var DecoratedTorusKnot4b = class extends Curve {
	...
	*/
	getPoint(t, optionalTarget = new Vector3()) {
		...
		const x = Math.cos(2 * fi) * (1 + .45 * Math.cos(3 * fi) + .4 * Math.cos(9 * fi));
		const y = Math.sin(2 * fi) * (1 + .45 * Math.cos(3 * fi) + .4 * Math.cos(9 * fi));
		const z = .2 * Math.sin(9 * fi);
		return point.set(x, y, z).multiplyScalar(this.scale);
	}
};
/**
...
var DecoratedTorusKnot5a = class extends Curve {
	...
	*/
	getPoint(t, optionalTarget = new Vector3()) {
		const point = optionalTarget;
		const fi = t * Math.PI * 2;
		const x = Math.cos(3 * fi) * (1 + .3 * Math.cos(5 * fi) + .5 * Math.cos(10 * fi));
		const y = Math.sin(3 * fi) * (1 + .3 * Math.cos(5 * fi) + .5 * Math.cos(10 * fi));
		const z = .2 * Math.sin(20 * fi);
		return point.set(x, y, z).multiplyScalar(this.scale);
	}
};
/**
...
var DecoratedTorusKnot5c = class extends Curve {
	...
	*/
	getPoint(t, optionalTarget = new Vector3()) {
		const point = optionalTarget;
		const fi = t * Math.PI * 2;
		const x = Math.cos(4 * fi) * (1 + .5 * (Math.cos(5 * fi) + .4 * Math.cos(20 * fi)));
		const y = Math.sin(4 * fi) * (1 + .5 * (Math.cos(5 * fi) + .4 * Math.cos(20 * fi)));
		const z = .35 * Math.sin(15 * fi);
		return point.set(x, y, z).multiplyScalar(this.scale);
	}
};
//#endregion
export { CinquefoilKnot, DecoratedTorusKnot4a, DecoratedTorusKnot4b, DecoratedTorusKnot5a, DecoratedTorusKnot5c, FigureEightPolynomialKnot, GrannyKnot, HeartCurve, HelixCurve, KnotCurve, TorusKnot, TrefoilKnot, TrefoilPolynomialKnot, VivianiCurve };
